...
exports.register = (req, res, next) => { // Middleware pour l'inscription
    ...
    const profilePicture = `${req.protocol}://${req.get("host")}/images/userProfilePictures/default.png`; // Photo de profil par défaut se trouvant dans le répertoire images du backend
        ...
        bcrypt
            ...
            .then((hash) => {
                if (validator.isEmail(req.body.email)) { // Si la forme de l'email est correcte, alors on crée le nouvel utilisateur
                    let sql = "INSERT INTO users (username, fname, lname, email, password, profilePictureUrl) VALUES (?, ?, ?, ?, ?, ?)"; // Commande mySQL à éxecuter
                    let inserts = [username, fname, lname, email, hash, profilePicture]; // Valeurs à insérer dans la base de données
                    sql = mysql.format(sql, inserts);
                    db.query(sql, (error, result) => { // Envoi des données à la BDD
                        if (!error) { // Si aucune erreur n'est retournée
                            res.status(201).json({
...
exports.login = (req, res, next) => { // Middleware pour la connexion
    ...
    if (validator.isEmail(credentials)) { // Si l'identifiant de l'utilisateur est un mail, alors on utilisera la commande SQL suivante
        sql = `SELECT * FROM users WHERE email = ?`;
    } else { // Sinon on utilisera la commande SQL suivante
    ...
    db.query(sql, (err, result) => {
        if (err) throw (err);
        if (result.length > 0) {
            bcrypt.compare(password, result[0].password) // On compare le hash du mot de passe entré avec celui enregistré dans la base de données
                .then(valid => {
                    if (!valid) { // Si il ne correspond pas au mot de passe de l'utilisateur, on retourne une erreur
                        console.log("Utilisateur introuvable")
                        return res.status(401).json({ error: "Nom d\'utilisateur ou mot de passe incorrect" });
                    } else { // Sinon l'utilsateur se connecte
                        console.log('User connected');
                        res.status(200).json({
                            user: { // On renvoie un objet avec les informations de l'utilisateur
                            ...
                            token: jwt.sign({ // On signe égalment le token qui permettra à l'utilisateur de naviguer sur le site
                                userId: result[0].userId,
                                roleId: result[0].roleId
                            },
...
exports.deleteAccount = (req, res, next) => { // Middleware pour la suppression du compte
    ...
    if (roleId == 1 || userId == userProfileId) { // Si l'utilisateur est admin ou que l'ID du profil est le même que celui de l'utilisateur authentifié
        ...
        db.query(selectUser, (error, result) => {
            ...
            if (profilePicture != `${req.protocol}://${req.get("host")}/images/userProfilePictures/default.png`) { // Si la photo de profil est différente de la photo par défaut
                const filename = result[0].profilePictureUrl.split("/images/userProfilePictures/uploads/")[1]; // On récupère le nom du fichier de la photo
                console.log("Nom du fichier: " + filename);
                fs.unlink(`images/userProfilePictures/uploads/${filename}`, () => { // On supprime la photo
...
exports.editProfilePicture = (req, res, next) => { // Modification de la photo de profil
    const errorCode = "409"
    ...
    if (roleId == 1 || userToEdit == userId) { // Si l'utilisateur est admin ou que l'ID du profil est le même que celui de l'utilisateur authentifié
        if (profilePicture != `${req.protocol}://${req.get("host")}/images/userProfilePictures/default.png`) { // Si la photo de profil n'est pas celle par défaut
            ...
            db.query(selectUser, (error, result) => {
                ...
                console.log("Nom du fichier: " + filename);
                fs.unlink(`images/userProfilePictures/uploads/${filename}`, () => { // On supprime l'ancienne photo de profil
...
exports.editAccount = (req, res, next) => { // Middleware pour la modification du compte
    ...
    if (username == null || email == null || fname == null || lname == null || password == null || username == "" || email == "" || fname == "" || lname == "" || password == "") {
        ...
        db.query(savedHash, (err, pass) => {
            bcrypt.compare(password, pass[0].password) // On compare le mot de passe avec celui déjà enregistré
                .then(valid => {
                    console.log(pass)
                    if (!valid) {
                        console.log("User not found")
                        ...
                        bcrypt // On enregistre le nouveau mot de passe
                            ...
                            .then((hash) => {
                                if (validator.isEmail(req.body.email)) {
                                    ...
                                    console.log(username + " fname " + fname + " lname " + lname + " email " + email + " hash " + hash + " userid " + userId) // On enregistre les nouvelles informations
                                    db.query(sql, (err, result) => {
                                        console.log(err);
                                        console.log(result);
                                        if (result) {
...
exports.editAccountAdmin = (req, res, next) => { // Middleware pour la modification du compte par un admin
    const token = req.headers.authorization.split(" ")[1];
    const decodedToken = jwt.verify(token, process.env.JWT_TOKEN);
    const roleId = decodedToken.roleId;
    const userId = decodedToken.userId
